...
/** Encabezado exactamente «Puntaje RSH» (mismo texto que el export del panel). */
export function findPuntajeRshColumnKey(headers: string[]): string | null {
  for (const h of headers) {
    if (normEncabezado(h) === 'puntaje rsh') return h
  }
  return null
}
...
/** Encabezado exactamente «Puntaje Hermanos» (mismo texto que el export del panel). */
export function findPuntajeHermanosColumnKey(headers: string[]): string | null {
  for (const h of headers) {
    if (normEncabezado(h) === 'puntaje hermanos') return h
  }
  return null
}
