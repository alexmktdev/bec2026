...
import { getCriterioDesempateConfig } from '../../services/filtroConfigService'
...
import {
  computeTotalesEstadoRevisionPlanilla,
  type TotalesEstadoRevisionPlanilla,
} from '../../utils/excelRevisionRowMatch'

export function Dashboard() {
  const { postulantes, loading, errorPostulantes, refrescarPostulantes, actualizarPostulanteLocal, eliminarPostulanteLocal } =
    useAdminFilter()
  ...
  const [planillaTotales, setPlanillaTotales] = useState<TotalesEstadoRevisionPlanilla | null>(null)
  const [rankingDesempateFilas, setRankingDesempateFilas] = useState<number | null>(null)
  const [statsLoading, setStatsLoading] = useState(false)

  const loadDashboardStats = useCallback(async () => {
    setStatsLoading(true)
    try {
      const [excel, criterio] = await Promise.all([
        loadExcelRevisionImportFirestore(),
        getCriterioDesempateConfig(),
      ])
      setPlanillaTotales(computeTotalesEstadoRevisionPlanilla(excel))
      const data = await obtenerRankingDesempate(criterio ?? null)
      setRankingDesempateFilas(data.postulantes.length)
    } catch (e) {
      console.error('Error cargando métricas del panel:', e)
      setPlanillaTotales(computeTotalesEstadoRevisionPlanilla(null))
      setRankingDesempateFilas(null)
    } finally {
  ...
  }, [loading, loadDashboardStats])

  async function handleRefrescarPanel() {
  ...
  const validadosPlanilla = planillaTotales?.validados ?? 0
  const rechazadosPlanilla = planillaTotales?.rechazados ?? 0
  const rankingFilas =
    rankingDesempateFilas == null ? (statsLoading ? null : 0) : rankingDesempateFilas
  ...
  return (
    <AdminLayout>
      <div className="flex-1 w-full px-4 py-8 sm:px-6 lg:px-8 space-y-6 max-w-[1600px] mx-auto">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {/* Doc. validada — columna Estado planilla Revisión de documentos */}
            <div className="flex flex-col justify-between rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm border-l-4 border-l-emerald-500">
              ...
              <p className="mt-3 text-4xl font-black text-slate-800 leading-none tabular-nums">
                {statsLoading && planillaTotales === null ? '–' : validadosPlanilla}
              </p>
              <p className="mt-2 text-xs text-slate-400">
                celdas «Validado» en la planilla de <span className="font-semibold text-slate-500">Revisión de documentos</span>
                {planillaTotales && !planillaTotales.tieneColumnaEstado && planillaTotales.totalFilas > 0
                  ? ' (sin columna Estado reconocida)'
                  : ''}
              </p>
            </div>

            {/* Rechazados — misma planilla */}
            <div className="flex flex-col justify-between rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm border-l-4 border-l-red-500">
              ...
              <p className="mt-3 text-4xl font-black text-slate-800 leading-none tabular-nums">
                {statsLoading && planillaTotales === null ? '–' : rechazadosPlanilla}
              </p>
              <p className="mt-2 text-xs text-slate-400">
                celdas «Rechazado» en la misma planilla de revisión
              </p>
            </div>

            {/* Ranking final — total tabla Filtro por desempate */}
            <div className="flex flex-col justify-between rounded-xl border-indigo-200 bg-indigo-50/30 px-5 py-4 shadow-sm border-l-4 border-l-indigo-600">
              <div className="flex items-start justify-between">
                <p className="text-[11px] font-bold uppercase tracking-widest text-indigo-700">Ranking final</p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <p className="mt-3 text-4xl font-black text-indigo-900 leading-none tabular-nums">
                {rankingFilas === null ? '–' : rankingFilas}
              </p>
              <p className="mt-2 text-xs text-indigo-500">
                total de filas en la tabla de <span className="font-semibold">Filtro por desempate</span> (criterio guardado)
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => void handleRefrescarPanel()}
        ...
        <div className="space-y-4">
          {errorPostulantes ? (
            ...
            <PostulantesTable
              ...
              onActualizar={(actualizado) => actualizarPostulanteLocal(actualizado)}
            />
          )}
        </div>
      </div>
      ...
      {selected && (
        <PostulanteDetail postulante={selected} onClose={() => setSelected(null)} />
      )}
    </AdminLayout>
  )
}
